...
export function buildDispatchEnvelope(
  ...
  return [
    ...
    "",
    "A scheduled dispatch has exactly ONE turn. There is no later turn, so a",
    "background-task completion notice can never reach you. Do NOT start a command",
    "in the background and then end your turn to wait for it. When you stop, the",
    "dispatch ends, the background command is killed, and the run records no",
    "outcome at all (`outcomeSource=none`, outcome `unknown`) — the work is lost",
    "AND the fleet loses the signal. Run the command in the foreground and wait",
    "for it, even when it costs several minutes of wall clock. If the work cannot",
