...
export function hygieneLauncherPath(home = routinesHome()): string {
  return join(home, "daemon", "run-hygiene.sh");
}

export function renderHygieneLauncher(): string {
  return `#!/usr/bin/env bash
# routines hygiene launchd entrypoint.
# Generated by \`routines install-hygiene\`; resolves the live CLI at runtime so
# the LaunchAgent is not pinned to the checkout that installed it.
set -euo pipefail

export ROUTINES_HOME="\${ROUTINES_HOME:-$HOME/.routines}"
export HOME="\${HOME:-$HOME}"
export USER="\${USER:-${process.env.USER ?? "tomtang"}}"
...
export function renderHygienePlist(opts: {
  ...
  const args = opts.direct
    ? [opts.program]
    : [runtime, opts.program, "hygiene", "--json", "--ff-install"];
...
export function installHygieneDaemon(opts: {
  ...
  const launcher = hygieneLauncherPath();
  mkdirSync(join(homedir(), "Library", "LaunchAgents"), { recursive: true });
  mkdirSync(daemonLogDir(), { recursive: true });
  writeFileSync(launcher, renderHygieneLauncher(), "utf8");
  chmodSync(launcher, 0o755);
  writeFileSync(p, renderHygienePlist({ ...opts, program: launcher, direct: true }));
  const uid = process.getuid?.() ?? 0;
  try {
    try {
